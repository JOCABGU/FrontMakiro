...
const donutStyle = (a: number, b: number, c: number): React.CSSProperties => {
  ...
  return {
    background: `conic-gradient(#2e7d32 0 ${pa}%, #0097a7 ${pa}% ${pa + pb}%, #c2185b ${pa + pb}% ${pa + pb + pc}%, #e5e7eb ${pa + pb + pc}% 100%)`,
  }
}
...
const NpsDashboardPage = () => {
  ...
  const metrics = useMemo(() => {
    ...
    daySeries.sort((a, b) => a.label.localeCompare(b.label, undefined, { numeric: true }))
  ...
  const maxDay = useMemo(() => Math.max(1, ...metrics.daySeries.map((x) => x.total)), [metrics.daySeries])
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
              ...
              <div key={d.label} className="flex flex-col items-center gap-1">
                <div className="w-full rounded bg-slate-100 p-1" style={{ height: 150 }}>
                    ...
                    <div className="relative flex h-full flex-1 items-end">
                      <div className="relative w-full rounded bg-blue-600" style={{ height: `${(d.prom / maxDay) * 100}%` }} title={`Promotor: ${d.prom}`}>
                        {d.prom > 0 ? <span className="absolute left-1/2 top-1 -translate-x-1/2 text-[10px] font-semibold text-black">{d.prom}</span> : null}
                      </div>
                    </div>
                    <div className="relative flex h-full flex-1 items-end">
                      <div className="relative w-full rounded bg-red-600" style={{ height: `${(d.det / maxDay) * 100}%` }} title={`Detractor: ${d.det}`}>
        ...
        <div className="rounded-xl border bg-white p-4">
          ...
          <div className="relative mx-auto h-40 w-40 rounded-full" style={donutStyle(metrics.prom, metrics.det, metrics.pas)}>
            <div className="absolute left-1/2 top-1/2 w-20 -translate-x-1/2 -translate-y-1/2 rounded-md bg-white/85 p-1 text-center text-[10px] font-semibold text-black leading-4">
              <div>P {metrics.prom}</div>
              <div>D {metrics.det}</div>
              <div>Pa {metrics.pas}</div>
            </div>
          </div>
